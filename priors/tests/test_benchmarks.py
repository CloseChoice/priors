...
def add_speed_result(dataset_size: str, num_trans: int, num_items: int, 
                    itemsets_found: int, execution_time: float, 
                    throughput: float, notes: str = ""):
...
def add_memory_result(dataset_size: str, peak_memory: float, 
                     dataset_memory: float, efficiency: float,
                     notes: str = ""):
...
def test_speed_scaling(num_trans, num_items, avg_size, min_support):
    ...
    # Store results
    add_speed_result(
        dataset_size, num_trans, num_items, 
        itemsets_found, execution_time, throughput,
        f"Support: {min_support}, Gen: {format_time(gen_time)}"
    )
    
    if psutil:
        add_memory_result(
            dataset_size, peak_memory, dataset_memory, 
            memory_efficiency,
...
@pytest.mark.parametrize("num_trans,num_items,density", [
...
def test_memory_efficiency(num_trans, num_items, density):
    ...
    for support in support_levels:
        ...
        if psutil and memory_used > expected_max_memory:
            print(f"  Warning: High memory usage ({format_memory(memory_used)} > {format_memory(expected_max_memory)})")
...
def print_speed_summary():
    ...
    # Print header
    print(f"{'Dataset Size':<15} {'Transactions':<15} {'Items':<6} {'Itemsets Found':<15} "
          f"{'Time':<10} {'Throughput':<15} {'Notes':<30}")
    print("-" * 120)
    
    # Print results
    for result in speed_results:
        print(f"{result['Dataset Size']:<15} {result['Transactions']:<15} "
...
def print_memory_summary():
    ...
    # Print header
    print(f"{'Dataset Size':<15} {'Dataset Memory':<15} {'Peak Memory':<15} "
          f"{'Efficiency':<12} {'Notes':<30}")
    print("-" * 100)
    
    # Print results
    for result in memory_results:
        print(f"{result['Dataset Size']:<15} {result['Dataset Memory']:<15} "
